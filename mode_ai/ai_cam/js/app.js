(function () {
  ...
  const CODE_MAP = { batu: '▓▓', gunting: '◈◈', kertas: '░░' };
  const NAME_MAP = { batu: 'BATU', gunting: 'GUNTING', kertas: 'KERTAS' };

  const CHOICE_TO_INT = { batu: 0, kertas: 1, gunting: 2 };
  ...
  function getAIChoice() {
    if (isLSTMReady && playerHistory.length >= 5) {
      const predicted = lstmModel.predict(playerHistory);
  ...
  async function detectionLoop() {
    ...
    try {
      ...
      detector.drawLandmarks(canvasEl, results, '#00F0FF');
      ...
      if (results && results.multiHandLandmarks && results.multiHandLandmarks.length) {
        if (!hasGuessed) {
          ...
          if (prediction && prediction.confidence >= 0.5) {
            ...
            const opponentGuess = getAIChoice();
            ...
            const choiceInt = CHOICE_TO_INT[playerGuess];
  ...
  if (buttonOpenCamera) {
    buttonOpenCamera.addEventListener('click', async function () {
      try {
        ...
        if (textInfo) textInfo.textContent = isLSTMReady ? 'AI Siap Ayok Mulai!' : 'Ayok Mulai!';
