(function () {
    ...
    card.addEventListener('click', function () {
      ...
      if (DISPLAY_OPPONENT) {
        ...
        setTimeout(function () {
          ...
          const choiceInt = CHOICE_TO_INT[userChoice];
  ...
  async function initLSTM() {
    ...
    if (RESULT_TEXT) {
      RESULT_TEXT.className = 'result-text font-display font-extrabold text-sm sm:text-base tracking-wider';
      RESULT_TEXT.textContent = 'Memuat Model...';
      RESULT_TEXT.classList.add('draw-text');
    }
    ...
    try {
      ...
      if (loaded) {
        ...
        if (RESULT_TEXT) {
          RESULT_TEXT.textContent = 'Ayo Mulai!';
      ...
      if (RESULT_TEXT) {
        RESULT_TEXT.textContent = 'GAGAL MEMUAT AI — mode random aktif';
        RESULT_TEXT.classList.add('lose-text');
      }
      isLSTMReady = false;
      setCardsEnabled(true);
